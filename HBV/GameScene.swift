...
struct PhysicsCategory {
    ...
    static let gc: UInt32 = 0x1 << 4
    static let gravity: UInt32 = 0x1 << 5
}

class GameScene: SKScene, SKPhysicsContactDelegate {
    
    let GClink = "https://www.amazon.es/g/7J39EHX7BZZM23?_encoding=UTF8&asin=B00FRC6LOU"
    ...
    let gc = SKSpriteNode(imageNamed: "amazon")
...
//                                self.enemyList.removeAll()
        ...
        if level == 3
            && body1.categoryBitMask & PhysicsCategory.ball != 0
            && body2.categoryBitMask & PhysicsCategory.gc != 0 {
            if contact.collisionImpulse > 30 {
                setScore(score: score + 100, fade: true)
                UIApplication.shared.open(URL(string: GClink)!, options: [:], completionHandler: nil)
            }
        }
    }
    ...
    func spawnLevel3() {
        run(SKAction.repeatForever(SKAction.playSoundFileNamed("Sounds/happy-birthday-music-box.mp3", waitForCompletion: true)))
        
        let hbLabel = SKLabelNode(fontNamed: "Chalkduster")
        hbLabel.text = "Happy Birthday 🎉"
        hbLabel.alpha = 0
        hbLabel.position = CGPoint(x: size.width * 0.5, y: size.height * 0.5)
        hbLabel.run(SKAction.fadeIn(withDuration: 1))
        
        gc.position = enemyList.first!.position
        gc.xScale = 0.2
        gc.yScale = 0.2
        gc.color = .blue
        gc.colorBlendFactor = 0.5
        gc.physicsBody = SKPhysicsBody(texture: gc.texture!, alphaThreshold: 0.5, size: gc.size)
        gc.physicsBody!.affectedByGravity = false
        
        gc.physicsBody!.categoryBitMask = PhysicsCategory.gc
        gc.physicsBody!.contactTestBitMask = PhysicsCategory.all
        gc.physicsBody!.collisionBitMask = PhysicsCategory.all
        
        addChild(hbLabel)
        addChild(gc)
        
        gravity.strength = -50
        gravity.region = SKRegion(radius: Float(size.width)/2)
        gravity.categoryBitMask = PhysicsCategory.gravity
        gravity.position = gc.position
        addChild(gravity)
        
        ball.physicsBody!.fieldBitMask = PhysicsCategory.gravity
        playerPaddle.physicsBody!.fieldBitMask = 0
        physicsBody!.fieldBitMask = 0
        gc.physicsBody!.fieldBitMask = 0
        
        /*run(SKAction.repeatForever(SKAction.sequence([
            SKAction.customAction(withDuration: 0, actionBlock: { (_, _) in
                let x = Double(arc4random_uniform(UInt32(self.size.width)))
                let y = Double(arc4random_uniform(UInt32(self.size.height/3))) + Double(self.size.height/3*2)
                self.gc.run(SKAction.move(to: CGPoint(x: x, y: y), duration: 1))
            }),
...
//        gc.run(SKAction.repeatForever(SKAction.follow(square.cgPath, asOffset: true, orientToPath: false, duration: 10)))
